...
const MODULES = {
  loop_architecture: {
    title: 'Loop Engineering / 有界智能体循环',
    description: '把开放式“继续思考”改造成目标级、证据驱动、预算可执行的 Plan → Act → Observe → Critic → Stop 状态机；业务核心保持不变。',
    prompt: '请做退款闭环：先查规则依据，再分析 237 笔交易，最后提交退款工单',
    steps: [
      {
        title: '定义状态与硬预算',
        body: 'LoopPolicy 在代码中固定最大迭代数、最大工具调用数和证据要求。预算是运行时保护，不依赖模型自觉停止。',
        code: 'LoopPolicy(max_iterations=3, max_tool_calls=4, require_evidence=True)',
        file: 'loop_engine.py',
      },
      {
        title: 'Plan：为每个步骤声明证据契约',
        body: '先识别 objective，再为规则查询、交易分析和工单创建声明 required_events、预计工具数、引用要求和副作用属性。',
      ...
      {
        title: 'Act + Observe：每轮推进一个步骤',
        body: '每轮只执行一个可验收动作，继续复用云搜索、MEM0、幂等工单、Sandbox、MCP、A2A 和安全规则；Observe 按真实能力事件累计工具用量。',
        code: 'result = self.business.chat(step.instruction, ...)\nloop.observe(tool_calls_observed=..., tool_calls_used=...)',
        file: 'loop_engine.py + demo_core.py',
      },
      {
        title: 'Critic + Stop：证据通过才进入下一步',
        body: 'Critic 对照 required_events、引用和安全状态验收。成功、安全拦截、预算不足、无进展或副作用不可证明都有独立 stop reason。',
        code: 'loop.critic(accepted=true, missing_evidence=[])\nloop.stop(reason="acceptance_criteria_met", iterations_used=3, tool_calls_used=4)',
        file: 'loop_engine.py',
      },
      {
        title: '在 AgentKit 混合云验证',
        body: '部署 Runtime 时开启观测服务；在线测试后到 Trace 分析核对一次 /invoke 中的 Agent、模型、工具 Span，并在本地 UI 核对 loop.* 业务事件。',
        code: 'agentkit launch\nagentkit status\n# 控制台：高级配置 → 观测服务 → 启用\n# 在线测试：POST /invoke',
        file: 'docs/loop_engineering.md + docs/runtime_deployment.md',
      },
    ],
  },
  runtime: {
  ...
  tools: {
    ...
    steps: [
      ...
      },
      {
        ...
        code: 'from veadk.tools.builtin_tools.run_code import run_code\n\nagent = Agent(\n    name="hybrid_cloud_loop_engineering",\n    tools=[loop_engineering_demo, run_code],\n)',
        file: 'agent.py',
      },
      {
        title: '仅在关联存在时启用',
  ...
  evaluation_dataset: {
    ...
    description: '评测集 hybrid_loop_engineering_runtime_core_v1 使用 input + reference_output：保留四条已验证基础链路，并新增一条 Loop 架构验收 Case。',
    prompt: '知识库验收标记是什么？请同时返回来源文件名。',
    steps: [
      {
        title: '导入版本化评测集',
        body: '在 Agent 评测中新建 hybrid_loop_engineering_runtime_core_v1，导入仓库里的 JSONL，并提交一个可用于实验的版本。',
        code: 'evaluation/hybrid_loop_engineering_runtime_core_v1.jsonl\n\n字段：input\n字段：reference_output',
  ...
  evaluator: {
    ...
    steps: [
      ...
      {
        title: '创建 Code 评估器',
        body: '创建 runtime_deterministic_checks_v1，复制仓库中完整的 exec_evaluation 实现，确定性检查五条链路的关键证据，包括 Loop 评审与停止原因。',
  ...
  experiment: {
    ...
    steps: [
      {
        title: '实验配置',
        body: '评测对象选择 AgentKit 智能体 hybrid-cloud-loop-engineering-demo-v2，评测集使用 hybrid_loop_engineering_runtime_core_v1 0.0.1，关联 LLM 和 Code 两个评估器。',
        code: '实验：jilidemo\n评测对象：hybrid-cloud-loop-engineering-demo-v2\n评测集：hybrid_loop_engineering_runtime_core_v1 0.0.1\n评估器：runtime_answer_correctness_v1 + runtime_deterministic_checks_v1',
      },
      {
        title: '字段映射',
      ...
      {
        title: '基础 Case 与新增 Loop Case',
        body: '前四条继承首个 Demo 的平台验证结论；新增 Loop Case 已纳入本地契约测试，部署新 Runtime 后需重新运行实验并保存平台结果。',
  ...
  observability: {
    ...
    prompt: '请明确调用 loop_engineering_demo，分析这 237 笔交易的总收益，并只返回关键计算结果。',
    steps: [
      {
        title: '平台开启观测服务',
...
function renderSessionMessages() {
  ...
  if (!session.messages.length) {
    addMessage('assistant', '你好，我会用有界的 Plan → Act → Observe → Critic → Stop 循环执行任务，并展示目标证据、真实预算与停止原因。', [], false);
    return;
  }
...
function renderInspector(data) {
  ...
  (data.events || []).forEach(event => {
    ...
    if (event.name === 'loop.iteration.start') counters.knowledge += 1;
    if (['tool','mcp','sandbox','skill','a2a'].some(prefix => event.name.startsWith(prefix))) counters.tools += 1;
    if (event.name === 'loop.stop') counters.security += 1;
  });
