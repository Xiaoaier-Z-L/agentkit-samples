...
app = FastAPI(title="Hybrid Cloud Loop Engineering Workbench")
...
def capabilities() -> dict[str, dict[str, str | bool]]:
    """Expose non-secret component status used by the local workbench."""
    from platform_capabilities import capability_status
...
def chat(request: LocalChatRequest) -> dict[str, object]:
    ...
    try:
        ...
        return {
            ...
            "thoughts": thoughts,
            "events": [
                {"name": "runtime.run_sse", "status": "succeeded", "mode": "live", "detail": {}}
            ],
        }
