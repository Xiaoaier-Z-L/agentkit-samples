...
def test_agent_tool_returns_json() -> None:
    payload = json.loads(loop_engineering_demo("上周买的理财产品可以退吗？"))
...
def test_adk_discovery_is_hidden_but_public_invoke_remains() -> None:
    ...
    def list_apps() -> list[str]:
        return ["hybrid_cloud_loop_engineering"]
...
def test_mcp_protocol_and_agent_card() -> None:
    ...
    assert initialized["result"]["serverInfo"]["name"] == "hybrid-loop-engineering"
