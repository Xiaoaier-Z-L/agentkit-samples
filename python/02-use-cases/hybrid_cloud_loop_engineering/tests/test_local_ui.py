...
def test_root_is_loop_engineering_guide_and_chat_is_workbench() -> None:
    client = TestClient(local_ui.app)

    guide = client.get("/")
    workbench = client.get("/chat")

    assert guide.status_code == 200
    assert "让 Agent 迭代" in guide.text
    assert "6 STAGES" in guide.text
    assert workbench.status_code == 200
    assert "Loop 闭环验收" in workbench.text
    capabilities = client.get("/api/capabilities")
    assert capabilities.status_code == 200
    assert "sandbox" in capabilities.json()
