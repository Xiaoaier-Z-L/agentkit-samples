...
> 状态说明：文中的 4 条实验数据来自首个客服 Demo 的已验证平台基线，用于证明公共 Runtime 链路可复用；本样例新增的第 5 条 Loop 架构 Case 已完成本地测试，但只有部署新的 Loop Runtime 并重新运行实验后，才能标记为混合云已验证。
...
- 评测集：`hybrid_loop_engineering_runtime_core_v1`，版本 `0.0.1`
- 数据项：4 条
...
1. 选择评测集 `hybrid_loop_engineering_runtime_core_v1` 的已提交版本。
2. 评测对象类型选择“AgentKit 智能体”。
3. 评测对象选择主 Runtime `hybrid-cloud-loop-engineering-demo-v2`。
