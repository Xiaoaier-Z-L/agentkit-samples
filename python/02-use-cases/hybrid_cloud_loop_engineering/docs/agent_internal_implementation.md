...
Runtime 是 Loop、知识库、记忆、会话、工具、身份和 Trace 的统一承载边界。请先按照 [《智能体运行时部署》](runtime_deployment.md) 完成 `hybrid` 模式发布，确认 Runtime 为 `Ready` 且 `POST /invoke` 能返回最终回答，再继续 Loop Engineering 验证。

## 1. 请求链路

```mermaid
flowchart TD
  U[调用方或控制台] --> G[Runtime 或 FastAPI]
  G --> I[身份与会话隔离]
...
def loop_engineering_demo(message: str, tenant_id: str = "demo-bank",
...
def build_agent():
    ...
    knowledge = build_platform_knowledge("hybrid_cloud_loop_engineering")
    memory = build_platform_memory("hybrid_cloud_loop_engineering")
    options = {}
    if knowledge:
    ...
    return Agent(
        name="hybrid_cloud_loop_engineering", instruction=INSTRUCTION,
        model_name=settings.model_name, model_api_key=settings.model_api_key,
        model_api_base=settings.model_api_base, tools=[loop_engineering_demo], **options,
    )
```

```bash
# 本地确定性模式：不需模型密钥
DEMO_MODE=demo python agent.py
...
- 长期记忆：MEM0；`after_agent_callback` 将已完成会话保存到 `LongTermMemory`。
...
tools = [loop_engineering_demo]
if os.getenv("AGENTKIT_TOOL_ID"):
    tools.append(run_code)

Agent(name="hybrid_cloud_loop_engineering", tools=tools, ...)
```
...
Skill 包在 `agentkit_skills/customer-service-compliance/SKILL.md`。Skills Sandbox 与 AIO Sandbox 是两种不同的工具：前者执行已发布的 Skill 工作流，后者执行隔离代码。一个 Runtime 只关联一个 Tool，平台以通用的 `AGENTKIT_TOOL_ID` 注入当前关联工具的 ID。样例同时暴露两个 VeADK 客户端函数，以支持操作者在请求中明确指定需要 `run_code` 或 `execute_skills`；用户没有明确指定时，Agent 必须先追问，不得猜测 Tool 类型。
...
```bash
agentkit skills validate --path agentkit_skills/customer-service-compliance
agentkit skills push --path agentkit_skills/customer-service-compliance \
