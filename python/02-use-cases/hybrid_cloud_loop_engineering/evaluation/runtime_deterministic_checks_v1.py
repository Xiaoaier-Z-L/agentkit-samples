...
def exec_evaluation(turn):
    ...
    elif "loop 为什么停止" in input_lower or "loop.critic" in reference_lower:
        required = ["237", "1284650", "loop.critic", "loop.stop", "acceptance_criteria_met"]
        normalized = actual_lower.replace(",", "")
        missing = [item for item in required if item not in normalized]
        score = 1.0 if not missing else 0.0
        reason = (
            "Loop 业务结果、评审事件和停止原因均正确。"
            if score
            else f"Loop 架构证据缺少：{', '.join(missing)}"
        )
