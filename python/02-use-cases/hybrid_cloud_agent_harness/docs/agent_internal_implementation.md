...
Runtime 是 Harness、知识库、记忆、会话、工具、身份和 Trace 的统一承载边界。请先按照 [《智能体运行时部署》](runtime_deployment.md) 完成 `hybrid` 模式发布，确认 Runtime 为 `Ready` 且 `POST /invoke` 能返回最终回答，再继续 Harness Engineering 验证。
...
Agent(name="hybrid_cloud_agent_harness", tools=tools, ...)
```
