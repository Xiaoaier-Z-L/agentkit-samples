...
def test_runtime_deployment_guide_uses_environment_specific_openapi_host() -> None:
    ...
    forbidden_development_host_label = "V" + "AE"
    assert forbidden_development_host_label not in readme
    assert forbidden_development_host_label not in deployment
...
def test_root_is_harness_specific_build_route_not_customer_demo() -> None:
    client = TestClient(local_ui.app)

    index = client.get("/")
    guide_js = client.get("/guide/app.js")

    assert index.status_code == 200
    assert "把“能运行的 Agent”" in index.text
    assert "治理流水线" in index.text
    assert "与客服基础 Demo 的边界" in index.text
    ...
    assert "人类掌舵、智能体执行" in index.text
    assert 'href="/docs"' not in index.text
    assert ">API</a>" not in index.text
    assert "一个客服智能体的建设路径" not in index.text
    assert "把意图与状态变成工件" in guide_js.text
    assert "每轮从新鲜上下文恢复" in guide_js.text
    assert "让角色在有界循环中协作" in guide_js.text
    assert "用机械反馈形成背压" in guide_js.text
    assert "让失败可以恢复而非重来" in guide_js.text
