...
app = FastAPI(title="AgentKit Hybrid Cloud Agent Harness Demo")
app.add_middleware(
    CORSMiddleware,
...
GUIDE_WEB_DIR = Path(__file__).resolve().parent / "guide_web"
...
def run_engineering_harness(request: EngineeringTaskRequest) -> dict[str, object]:
    return engineering_service.run(request.objective, run_id=request.run_id).to_dict()


@app.get("/.well-known/agent-card.json")
def agent_card() -> dict[str, object]:
    return {
        "name": "hybrid-cloud-agent-harness",
        "description": "Policy-governed Agent Harness on AgentKit hybrid cloud",
        "url": "/a2a",
        "version": "0.3.0",
...
async def mcp_endpoint(request: Request) -> dict[str, object]:
    ...
    if method == "initialize":
        result = {
            ...
            "capabilities": {"tools": {}},
            "serverInfo": {"name": "hybrid-agent-harness", "version": "0.3.0"},
        }
