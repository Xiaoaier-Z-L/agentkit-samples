...
const a2uiMode = document.querySelector('#a2ui-mode');
const counters = { knowledge: 0, tools: 0, security: 0 };
const traceDialog = document.querySelector('#trace-dialog');
const traceHistory = document.querySelector('#trace-history');
const traces = [];
const sessionList = document.querySelector('#session-list');
const SESSION_STORE = 'agentkit-workbench-sessions-v2';
...
const MODULES = {
  harness_architecture: {
    title: 'Agent Harness / 生产运行包络',
    description: 'Harness 把身份、路由级最小权限、逐次工具预算、输出契约和递归脱敏从业务 Agent 中抽离；任一治理证明失败都会 fail closed。',
    prompt: '分析这 237 笔交易的总收益，并展示 Harness 对工具调用和输出做了哪些检查',
    steps: [
      {
        title: '标准化请求上下文',
        body: 'Runtime 网关完成 API Key/OAuth JWT 认证后，中间层把 tenant_id、user_id、session_id 和 identity_source 绑定到请求级 ContextVar。模型可见工具只有 message 参数，不能伪造身份。',
        code: 'RequestAuthorizationMiddleware → request_identity()\nagent_harness_demo(message)  # no model-supplied identity\nharness.request.accept → harness.identity.bind',
        file: 'harness.py + platform_request_context.py',
      },
      {
        title: '编译路由最小权限',
        body: 'HarnessPolicy 的全局注册表与 RoutePolicy 的路由权限取交集，同时取更严格的预算。预检结果在业务 Agent 执行前固定并写入事件。',
        code: 'effective_tools = route.allowed_tools & policy.allowed_tools\neffective_budget = min(route.max_tool_calls, policy.max_tool_calls)',
        file: 'harness.py',
      },
      {
        title: '执行已验证业务 Agent',
        body: 'Harness 调用原有 HybridCustomerService，继续复用云搜索、MEM0、会话、幂等工单、Sandbox、MCP、Skill 与 A2A；不重写已通过验证的业务决策。',
        code: 'result = self.agent.chat(...)',
        file: 'harness.py + demo_core.py',
      },
      {
        title: '执行证明与 fail-closed',
        body: '执行后逐次核对实际能力事件，不按工具名去重。未知工具、跨路由调用、超预算、空回答或缺失知识引用都会阻断原始结果。',
        code: 'unregistered | route_denied | budget_exceeded\n→ harness.tools.authorize\n→ harness.output.validate\n→ deny original result',
        file: 'harness.py',
      },
      {
        title: '递归脱敏 Trace 汇聚',
        body: '回答、引用与事件 detail 会在导出前递归脱敏，并记录 redacted_fields 与最终 allow/deny。平台 Runtime 开启观测后可核对治理事件和工具 Span。',
        code: 'harness.telemetry.export(\n  redaction="authorization-and-secrets",\n  redacted_fields=N, governance_decision="allow|deny"\n)',
        file: 'harness.py + docs/agent_harness.md',
      },
    ],
  },
  runtime: {
    ...
    steps: [
      {
        title: '准备本地环境与访问凭据',
        body: '本地安装 Python 3.10+ 与 uv，准备目标环境的 Region、云管理平台 AK/SK，以及模型 Endpoint、API Key 和 API Base。所有真实凭据只通过当前终端环境变量传入，不写入源码、脚本或 agentkit.yaml。',
      ...
      },
      {
        ...
        body: '复制并执行仓库中的 CLI 配置模板。COMMON_HOST 必须替换为当前云管理平台用户端对应的 AgentKit OpenAPI 域名；不同用户环境的域名不同，不能写死成某个演示环境。',
        ...
        note: '模板从 VOLCENGINE_ACCESS_KEY、VOLCENGINE_SECRET_KEY 和 VOLCENGINE_REGION 读取敏感配置。',
      },
      {
        title: '检查 Runtime 部署配置',
        body: '复制公开模板生成本地 agentkit.yaml，并按当前模型服务补齐三个模型变量。部署模式保持 hybrid；agentkit.yaml 已被仓库忽略，不应提交。',
        code: 'cp agentkit.yaml.example agentkit.yaml\nagentkit config\n\n# 必填模型变量\nMODEL_AGENT_NAME=<model-endpoint-id>\nMODEL_AGENT_API_KEY=<model-api-key>\nMODEL_AGENT_API_BASE=<model-api-base>',
        file: 'agentkit.yaml.example',
      },
      {
        title: '构建并部署 Runtime',
        body: '在项目根目录执行 launch。hybrid 模式会在本地构建镜像并上传到目标 CR，再由 AgentKit 创建或更新智能体运行时。首次部署通常需要 2～3 分钟。',
        code: 'agentkit launch\nagentkit status',
        note: '只有 status 显示 Ready，且 Runtime 日志没有启动错误，才进入在线测试。生产镜像架构必须为 linux/amd64。',
      },
      {
        title: '在线测试、观测与清理',
        body: '在控制台在线测试 /invoke，或把 Runtime Endpoint 与 API Key 填入本地 UI 的“连接配置”。请求成功后继续在可观测页核对 Trace。只有明确不再使用该实例时才执行 destroy。',
  ...
  evaluation_dataset: {
    ...
    description: '评测集 hybrid_agent_harness_runtime_core_v1 使用 input + reference_output：保留四条已验证基础链路，并新增一条 Harness 架构验收 Case。',
    prompt: '知识库验收标记是什么？请同时返回来源文件名。',
    steps: [
      {
        title: '导入版本化评测集',
        body: '在 Agent 评测中新建 hybrid_agent_harness_runtime_core_v1，导入仓库里的 JSONL，并提交一个可用于实验的版本。',
        code: 'evaluation/hybrid_agent_harness_runtime_core_v1.jsonl\n\n字段：input\n字段：reference_output',
  ...
  evaluator: {
    ...
    steps: [
      ...
      {
        title: '创建 Code 评估器',
        body: '创建 runtime_deterministic_checks_v1，复制仓库中完整的 exec_evaluation 实现，确定性检查五条链路的关键证据，包括 Harness 身份、策略、工具授权、输出和遥测。',
        code: '已验证代码：evaluation/runtime_deterministic_checks_v1.py\n入口格式：def exec_evaluation(turn):\n返回格式：return EvalOutput(score=score, reason=reason)',
  ...
  experiment: {
    ...
    steps: [
      {
        title: '实验配置',
        body: '评测对象选择 AgentKit 智能体 hybrid-cloud-agent-harness-demo-v2，评测集使用 hybrid_agent_harness_runtime_core_v1 0.0.1，关联 LLM 和 Code 两个评估器。',
        code: '实验：jilidemo\n评测对象：hybrid-cloud-agent-harness-demo-v2\n评测集：hybrid_agent_harness_runtime_core_v1 0.0.1\n评估器：runtime_answer_correctness_v1 + runtime_deterministic_checks_v1',
      },
      {
        title: '字段映射',
      ...
      {
        title: '基础 Case 与新增 Harness Case',
        body: '前四条继承首个 Demo 的平台验证结论；新增 Harness Case 已纳入本地契约测试，部署新 Runtime 后需重新运行实验并保存平台结果。',
  ...
  observability: {
    ...
    prompt: '请明确调用 agent_harness_demo，分析这 237 笔交易的总收益，并只返回关键计算结果。',
    steps: [
      {
        title: '平台开启观测服务',
...
function loadSessions() {
  ...
  return [{ id: 'workbench-001', title: '默认演示', messages: [] }];
}
...
fetch('/api/capabilities').then(r => r.json()).then(items => {
  const configured = Object.entries(items).filter(([, value]) => value.configured).length;
  document.querySelector('#runtime-endpoint').textContent = `${configured}/6 能力已配置`;
}).catch(() => {});
...
async function refreshRuntimeConfig() {
  ...
  const missingKey = config.remote && !config.api_key_configured;
  document.querySelector('#runtime-mode').textContent = missingKey
    ? 'Runtime Key Missing'
    : (config.remote ? 'Remote Runtime' : 'Demo Runtime');
  document.querySelector('#runtime-endpoint').textContent = missingKey
    ? '在 .env 设置 RUNTIME_API_KEY 后重启 UI'
    : config.endpoint;
  return config;
}
...
runtimeConfigForm.addEventListener('submit', async event => {
  ...
  runtimeConfigStatus.className = 'runtime-config-status'; runtimeConfigStatus.textContent = '已保存到本地 BFF 内存，可开始发送消息。';
  document.querySelector('#runtime-config-key').value = '';
...
document.querySelector('#runtime-config-demo').addEventListener('click', async () => {
  ...
  await refreshRuntimeConfig(); runtimeConfigDialog.close();
});
...
function renderSessionMessages() {
  ...
  if (!session.messages.length) {
    addMessage('assistant', '你好，我在 Agent Harness 中运行；身份、策略、工具预算、输出校验和 Trace 都由统一运行包络治理。', [], false);
    return;
  }
...
function renderInspector(data) {
  traceBadge.textContent = data.trace_id || 'No trace';
  timeline.innerHTML = '';
  counters.knowledge = 0; counters.tools = 0; counters.security = 0;
  (data.events || []).forEach(event => {
    const node = document.createElement('div');
    node.className = `event ${event.status === 'blocked' ? 'blocked' : ''}`;
    const title = document.createElement('strong'); title.textContent = event.name;
    const detail = Object.keys(event.detail || {}).length ? ` · ${JSON.stringify(event.detail)}` : '';
    const meta = document.createElement('small'); meta.textContent = `${event.status} · ${event.mode}${detail}`;
    node.append(title, meta); timeline.append(node);
    if (['harness.identity.bind','harness.policy.precheck','harness.output.validate'].includes(event.name)) counters.knowledge += 1;
    if (event.name === 'harness.tools.authorize') counters.tools += (event.detail?.authorized || []).length;
    if (event.name === 'harness.telemetry.export') counters.security += 1;
  });
  document.querySelector('#metric-knowledge').textContent = counters.knowledge;
  document.querySelector('#metric-tools').textContent = counters.tools;
  document.querySelector('#metric-security').textContent = counters.security;
  citations.innerHTML = '';
  if (!(data.citations || []).length) citations.textContent = '暂无引用';
  (data.citations || []).forEach(item => {
    const card = document.createElement('div'); card.className = 'citation-card';
    card.textContent = `${item.title} · ${item.chunk_id}`; citations.append(card);
  });
}

function normalizeA2UI(payload) {
  if (payload.version !== '0.9' || !Array.isArray(payload.components)) {
    throw new Error('A2UI 响应格式无效');
  }
  const output = {answer: '', citations: [], thoughts: [], events: [], trace_id: ''};
  payload.components.forEach(component => {
    const props = component?.props || {};
    if (component.type === 'text' && typeof props.text === 'string') output.answer = props.text;
    if (component.type === 'citation-list' && Array.isArray(props.items)) output.citations = props.items;
    if (component.type === 'reasoning' && Array.isArray(props.items)) output.thoughts = props.items;
    if (component.type === 'capability-timeline' && Array.isArray(props.events)) {
      output.events = props.events;
      output.trace_id = typeof props.trace_id === 'string' ? props.trace_id : '';
    }
  });
  if (!output.answer || !output.trace_id) throw new Error('A2UI 缺少 text 或 capability-timeline 组件');
  return output;
}
...
function renderTraceHistory() {
  ...
  if (!traces.length) {
    traceHistory.textContent = '尚未发送请求。';
    traceHistory.className = 'trace-history empty-state';
    return;
  }
  traceHistory.className = 'trace-history';
  [...traces].reverse().forEach(trace => {
    const card = document.createElement('article'); card.className = 'trace-card';
    const heading = document.createElement('h3'); heading.textContent = trace.trace_id || 'No trace id';
    const prompt = document.createElement('p'); prompt.textContent = `请求：${trace.message}`;
    const events = document.createElement('div'); events.className = 'trace-events';
    (trace.events || []).forEach(event => {
      const row = document.createElement('div'); row.className = 'trace-event';
      const name = document.createElement('strong'); name.textContent = event.name;
      const meta = document.createElement('small');
      meta.textContent = `${event.status} · ${event.mode}${Object.keys(event.detail || {}).length ? ` · ${JSON.stringify(event.detail)}` : ''}`;
      row.append(name, meta); events.append(row);
    });
    card.append(heading, prompt, events); traceHistory.append(card);
  });
}
...
async function send(message) {
  ...
  try {
    ...
    if (activeSessionId === requestSessionId) {
      traces.push({message, trace_id: completed.trace_id, events: completed.events || []});
...
function openModule(module) {
  ...
  if (!item) return;
  document.querySelector('#module-title').textContent = item.title;
  document.querySelector('#module-description').textContent = item.description;
  const steps = document.querySelector('#module-steps');
  steps.innerHTML = '';
  const flow = item.steps || [{ title: '实现摘要', body: item.description, code: item.code }];
  flow.forEach((step, index) => {
    const card = document.createElement('article');
    card.className = 'module-step';
    ...
    if (step.file || step.note) {
      const meta = document.createElement('small');
      meta.className = 'step-meta';
      meta.textContent = [step.file && `代码：${step.file}`, step.note].filter(Boolean).join(' · ');
      content.append(meta);
    }
    card.append(number, content);
    steps.append(card);
  });
  document.querySelector('#module-prompt').textContent = item.prompt;
  document.querySelector('#module-run').onclick = () => { moduleDialog.close(); send(item.prompt); };
  moduleDialog.showModal();
}

document.querySelectorAll('button.nav-item').forEach(button => button.addEventListener('click', async () => {
  document.querySelectorAll('button.nav-item').forEach(item => item.classList.remove('active'));
  button.classList.add('active');
  if (button.dataset.view === 'trace') openTrace();
  if (button.dataset.module) openModule(button.dataset.module);
  if (button.dataset.action === 'runtime-config') {
    const config = await refreshRuntimeConfig();
    document.querySelector('#runtime-config-endpoint').value = config.remote ? config.endpoint : '';
    document.querySelector('#runtime-config-key').value = '';
    runtimeConfigStatus.className = 'runtime-config-status';
...
traceBadge.addEventListener('click', openTrace);
traceDialog.addEventListener('click', event => { if (event.target === traceDialog) traceDialog.close(); });
document.querySelector('#module-close').addEventListener('click', () => moduleDialog.close());
