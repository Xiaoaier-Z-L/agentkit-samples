...
| 项目 | 平台对象 |
|---|---|
| 评测集 | `hybrid_agent_harness_runtime_core_v1` |
| LLM 评估器 | `runtime_answer_correctness_v1` |
| Code 评估器 | `runtime_deterministic_checks_v1` |
| 评测对象 | `hybrid-cloud-agent-harness-demo-v2` |
| 已验证实验 | `jilidemo` |

评测集包含 5 条链路：知识库 Canary、安全拒答、Sandbox `run_code` 数值计算、A2A 数据 Agent 委派，以及 Harness 治理链。前四条继承首个 Demo 的平台验证基线；第五条是本样例新增的架构验收 Case，部署新 Runtime 后需重新运行平台实验。
...
- 每条数据均应产生 `actual_output`；新增 Harness Case 还必须出现 identity、policy、tool authorization、output validation 与 telemetry 证据。
- Code 评估器 4/4 通过。
