...
const MODULES = {
  ...
  evaluator: {
    ...
    steps: [
      ...
      {
        title: '创建 Code 评估器',
        body: '创建 runtime_deterministic_checks_v1，复制仓库中完整的 exec_evaluation 实现，确定性检查四条链路的关键证据。',
...
function renderSessionMessages() {
  ...
  if (!session.messages.length) {
    addMessage('assistant', '你好，我可以演示知识检索、跨会话记忆、交易分析、安全防护和多 Agent 协同。', [], false);
    return;
  }
...
function renderInspector(data) {
  ...
  (data.events || []).forEach(event => {
    ...
    const meta = document.createElement('small'); meta.textContent = `${event.status} · ${event.mode}`;
    node.append(title, meta); timeline.append(node);
    if (event.name.startsWith('knowledge')) counters.knowledge += 1;
    if (['tool','mcp','sandbox','skill','a2a'].some(prefix => event.name.startsWith(prefix))) counters.tools += 1;
    if (event.name.startsWith('security')) counters.security += 1;
  });
