...
The agent deliberately uses the sample's deterministic complaint data rather
than the customer-service model.  This makes A2A routing verifiable without
mixing it with the main Agent's Knowledge, Memory or Session integrations.
"""
...
class ComplaintTrendAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        await updater.add_artifact(
            ...
            name=SKILL_ID,
            metadata={"agent": "hybrid-cloud-complaint-data-agent"},
        )
...
def build_app() -> FastAPI:
    ...
    app.get("/health")(lambda: {"status": "ok", "agent": card.name})
